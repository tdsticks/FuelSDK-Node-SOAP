...
var FuelSoap = function(options) {
    console.log("::fuel-soap:: FuelSoap:");
...
FuelSoap.prototype.soapRequest = function(options, callback) {
    ...
    this.AuthClient.getAccessToken(clone(options.auth), function(err, body) {
        ...
        // console.log("\n:fuel-soap: options.req:", options.req.CreateRequest.Objects);

        requestOptions.body = this._buildEnvelope(options.req, body.accessToken);

        // console.log("\n:fuel-soap: requestOptions.body:", requestOptions.body);
        // console.log("\n", requestOptions.body);
...
FuelSoap.prototype.create = function(type, props, options, callback) {
    ...
    // console.log("\n body.CreateRequest.Objects:", body.CreateRequest.Objects);
...
FuelSoap.prototype.retrieve = function(type, props, options, callback) {
    //console.log("::fuel-soap:: retrieve:");
    ...
    if(arguments.length < 4) {
        ...
        if(typeof arguments[2] === 'function') {
            ...
            if(isPlainObject(arguments[1])) {
                // console.log("   check if props or filter");
                ...
                // console.log("   ELSE: check if props or filter");
                clientIDs = null;
                filter    = null;
                options   = null;
            }
        ...
        if (options.hasOwnProperty("clientIDs")) {
    ...
    // console.log(":fuel-soap.retrieve: continueRequest:", continueRequest);
    ...
    if(continueReq) {
        body.RetrieveRequestMsg.RetrieveRequest.ContinueRequest = continueReq; // continueRequest == the RequestID
    }
 ...
 * @param {FuelSoap~StandardCallback} callback - Callback that handles response
 */
FuelSoap.prototype.perform = function(type, action, props, callback) {
    // console.log("FuelSoap.prototype.perform", type, action, def);

    // props = "CustomerKey": externalKeyParam
    var def = props;
...
FuelSoap.prototype._parseResponse = function(key, body, callback) {
    ...
    // console.log(":FuelSoap.prototype._parseResponse: key", key);
    // console.log(":FuelSoap.prototype._parseResponse: body", body);
    ...
    parseString(body, parseOptions, function(err, res) {
        ...
        if(key === 'RetrieveResponseMsg') {
            if(parsedRes.OverallStatus === 'OK' || parsedRes.OverallStatus === 'MoreDataAvailable') {
                ...
                console.error("ERROR: parsedRes:", parsedRes);
                console.error("ERROR: parsedRes.OverallStatus:", parsedRes.OverallStatus);
